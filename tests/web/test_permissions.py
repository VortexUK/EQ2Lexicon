...
def test_require_capability_rejects_unknown_capability_at_definition_time():
    ...
    from web.auth_deps import require_capability
...
async def test_admin_passes_without_touching_role_tables(app):
    ...
    path stays cheap."""
    with (
        patch("web.auth_deps.require_user_session", return_value={"id": "admin-1", "username": "admin"}),
        patch("web.auth_deps.is_admin", return_value=True),
        patch("web.auth_deps.users_db.user_has_capability_via_db", new_callable=AsyncMock) as m_db,
        patch("web.auth_deps.users_db.role_has_capability", new_callable=AsyncMock) as m_role,
        patch("web.routes.raid_strategies.zones_db.find_by_name", return_value=_fake_zone()),
        patch(
            "web.routes.raid_strategies._write_strategy_sync",
            return_value={
                "id": 1,
...
async def test_db_role_with_capability_passes(app):
    ...
    with (
        patch("web.auth_deps.require_user_session", return_value={"id": "contrib-7", "username": "contrib"}),
