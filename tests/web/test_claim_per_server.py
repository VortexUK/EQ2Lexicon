...
async def test_approved_claim_invisible_to_different_world():
    ...
    claim = await submit_claim(uid, "SomeVarsoonChar", world="Varsoon", path=_PATH)
    await review_claim(claim["id"], "approved", "admin-1", path=_PATH)

    varsoon_data = await get_active_claims(uid, world="Varsoon", path=_PATH)
    wuoshi_data = await get_active_claims(uid, world="Wuoshi", path=_PATH)
...
async def test_pending_claim_invisible_to_different_world():
    ...
    await submit_claim(uid, "WuoshiPendingChar", world="Wuoshi", path=_PATH)
...
async def test_primary_is_independent_per_world():
    ...
    claim_v = await submit_claim(uid, "VarsoonPrimary", world="Varsoon", path=_PATH)
    claim_v_row = await review_claim(claim_v["id"], "approved", "admin-1", path=_PATH)
    assert claim_v_row is not None

    claim_w = await submit_claim(uid, "WuoshiPrimary", world="Wuoshi", path=_PATH)
    claim_w_row = await review_claim(claim_w["id"], "approved", "admin-1", path=_PATH)
    assert claim_w_row is not None

    # auto-primary should already be set (first approval per world) — verify
    v_data = await get_active_claims(uid, world="Varsoon", path=_PATH)
    w_data = await get_active_claims(uid, world="Wuoshi", path=_PATH)
    ...
    ok = await set_primary(uid, claim_v["id"], world="Varsoon", path=_PATH)
    assert ok

    v_data2 = await get_active_claims(uid, world="Varsoon", path=_PATH)
    w_data2 = await get_active_claims(uid, world="Wuoshi", path=_PATH)
...
async def test_setting_varsoon_primary_does_not_affect_wuoshi_primary():
    ...
    c_v1 = await submit_claim(uid, "VarsoonA", world="Varsoon", path=_PATH)
    await review_claim(c_v1["id"], "approved", "admin-1", path=_PATH)

    c_v2 = await submit_claim(uid, "VarsoonB", world="Varsoon", path=_PATH)
    await review_claim(c_v2["id"], "approved", "admin-1", path=_PATH)

    # Approve one Wuoshi char
    c_w = await submit_claim(uid, "WuoshiC", world="Wuoshi", path=_PATH)
    await review_claim(c_w["id"], "approved", "admin-1", path=_PATH)

    # Confirm Wuoshi primary is set
    w_before = await get_active_claims(uid, world="Wuoshi", path=_PATH)
    assert w_before["approved"][0]["is_primary"] == 1

    # Switch Varsoon primary to VarsoonB
    ok = await set_primary(uid, c_v2["id"], world="Varsoon", path=_PATH)
    assert ok

    # Wuoshi primary must be unchanged
    w_after = await get_active_claims(uid, world="Wuoshi", path=_PATH)
...
async def test_submit_claim_records_world():
    ...
    claim = await submit_claim(uid, "RecordChar", world="Wuoshi", path=_PATH)
...
async def test_submit_claim_default_world_is_varsoon():
    ...
    claim = await submit_claim(uid, "VarsoonDefaultChar", world="Varsoon", path=_PATH)
...
async def test_withdraw_primary_promotes_within_same_world():
    ...
    c_v1 = await submit_claim(uid, "VarsoonFirst", world="Varsoon", path=_PATH)
    await review_claim(c_v1["id"], "approved", "admin-1", path=_PATH)

    c_v2 = await submit_claim(uid, "VarsoonSecond", world="Varsoon", path=_PATH)
    await review_claim(c_v2["id"], "approved", "admin-1", path=_PATH)

    # One Wuoshi char
    c_w = await submit_claim(uid, "WuoshiFirst", world="Wuoshi", path=_PATH)
    await review_claim(c_w["id"], "approved", "admin-1", path=_PATH)

    # Confirm initial state
    v_before = await get_active_claims(uid, world="Varsoon", path=_PATH)
    assert v_before["approved"][0]["is_primary"] == 1  # VarsoonFirst is primary

    # Withdraw the Varsoon primary
    changed = await withdraw_claim(c_v1["id"], uid, world="Varsoon", path=_PATH)
    assert changed

    # VarsoonSecond should now be primary on Varsoon
    v_after = await get_active_claims(uid, world="Varsoon", path=_PATH)
...
async def test_list_claims_with_world_filter():
    ...
    from web.db import list_claims
    ...
    await submit_claim(uid_v, "ListVarsoonChar", world="Varsoon", path=_PATH)
    await submit_claim(uid_w, "ListWuoshiChar", world="Wuoshi", path=_PATH)

    varsoon_pending = await list_claims(status="pending", world="Varsoon", path=_PATH)
    wuoshi_pending = await list_claims(status="pending", world="Wuoshi", path=_PATH)
    all_pending = await list_claims(status="pending", world=None, path=_PATH)
...
async def test_same_character_name_claimable_on_different_servers():
    ...
    await submit_claim("u1", "Sihtric", world="Varsoon", path=_PATH)

    # u2 can claim "Sihtric" on Wuoshi (different character / server) — must NOT raise
    await submit_claim("u2", "Sihtric", world="Wuoshi", path=_PATH)

    # but u2 claiming "Sihtric" on Varsoon (same server as u1's) must be rejected
    with pytest.raises(ValueError):
        await submit_claim("u2", "Sihtric", world="Varsoon", path=_PATH)
