...
async def test_mint_returns_raw_token_once(app):
    ...
    with (
        patch("web.routes.auth_tokens._require_user", _fake_session_user),
        patch(
            "web.routes.auth_tokens.users_db.mint_api_token",
...
async def test_mint_rejects_empty_name(app):
    with patch("web.routes.auth_tokens._require_user", _fake_session_user):
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            # Empty after strip
...
async def test_list_returns_users_tokens(app):
    ...
    with (
        patch("web.routes.auth_tokens._require_user", _fake_session_user),
        patch(
            "web.routes.auth_tokens.users_db.list_api_tokens",
...
async def test_revoke_success(app):
    with (
        patch("web.routes.auth_tokens._require_user", _fake_session_user),
        patch(
            "web.routes.auth_tokens.users_db.revoke_api_token",
...
def tmp_users_db(tmp_path, monkeypatch):
    ...
    # init_db creates schema; we point at the temp path.
...
async def test_db_mint_then_lookup_then_revoke(tmp_users_db):
    ...
    raw, row = await users_db.mint_api_token("user-123", "First", path=tmp_users_db)
    assert raw.startswith("eq2c_")
    assert row["name"] == "First"
    assert row["revoked_at"] is None

    # Lookup with raw → returns user info
    found = await users_db.lookup_api_token(raw, path=tmp_users_db)
    assert found is not None
    assert found["user_id"] == "user-123"
    assert found["discord_name"] == "Alice"

    # Revoke
    ok = await users_db.revoke_api_token("user-123", row["id"], path=tmp_users_db)
    assert ok is True

    # Revoked token no longer resolves
    after = await users_db.lookup_api_token(raw, path=tmp_users_db)
...
async def test_db_lookup_rejects_garbage(tmp_users_db):
    ...
    assert await users_db.lookup_api_token("", path=tmp_users_db) is None
    assert await users_db.lookup_api_token("not-our-prefix-foo", path=tmp_users_db) is None
    assert await users_db.lookup_api_token("eq2c_nonexistent", path=tmp_users_db) is None
...
async def test_db_revoke_scoped_to_user(tmp_users_db):
    ...
    raw_alice, row_alice = await users_db.mint_api_token("user-123", "Alice's", path=tmp_users_db)
    # Bob tries to revoke Alice's token
    ok = await users_db.revoke_api_token("user-456", row_alice["id"], path=tmp_users_db)
    assert ok is False
