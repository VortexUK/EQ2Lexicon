...
async def test_get_zone_returns_hydrated_payload(app):
    with patch("web.routes.zones.zones_db.find_by_name", return_value=_fake_zone()):
...
async def test_get_zone_unknown_name_returns_404(app):
    with patch("web.routes.zones.zones_db.find_by_name", return_value=None):
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
...
async def test_list_zones_by_expansion_uses_type_default(app):
    ...
    with patch(
        "web.routes.zones.zones_db.list_by_expansion",
...
async def test_list_zones_empty_type_disables_type_filter(app):
    ...
    with patch("web.routes.zones.zones_db.list_by_expansion", return_value=[]) as m:
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
...
async def test_list_zones_by_type_only(app):
    ...
    with patch(
        "web.routes.zones.zones_db.list_by_type",
