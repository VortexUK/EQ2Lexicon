...
async def test_submit_role_request_rejects_unknown_role(app):
    with patch("web.routes.role_requests.require_user_session", _fake_session_user):
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
...
async def test_submit_role_request_rejects_when_user_already_has_role(app):
    ...
    with (
        patch("web.routes.role_requests.require_user_session", _fake_session_user),
        patch("web.routes.role_requests.has_role", return_value=True),
    ):
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            r = await client.post("/api/me/role-requests", json={"role": "contributor"})
...
async def test_submit_role_request_409_when_pending_exists(app):
    ...
    with (
        patch("web.routes.role_requests.require_user_session", _fake_session_user),
        patch("web.routes.role_requests.has_role", return_value=False),
        patch(
            "web.routes.role_requests.create_role_request",
...
async def test_submit_role_request_writes_and_returns_row(app):
    ...
    with (
        patch("web.routes.role_requests.require_user_session", _fake_session_user),
        patch("web.routes.role_requests.has_role", return_value=False),
        patch("web.routes.role_requests.create_role_request", return_value=7) as m_create,
        patch("web.routes.role_requests.list_role_requests", return_value=[fresh]),
    ):
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            r = await client.post(
...
async def test_list_my_role_requests_returns_only_my_history(app):
    ...
    with (
        patch("web.routes.role_requests.require_user_session", _fake_session_user),
        patch(
            "web.routes.role_requests.list_role_requests",
...
async def test_withdraw_request_404_when_not_owned_or_resolved(app):
    ...
    with (
        patch("web.routes.role_requests.require_user_session", _fake_session_user),
        patch("web.routes.role_requests.withdraw_role_request", return_value=False),
    ):
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
...
async def test_withdraw_request_happy_path(app):
    with (
        patch("web.routes.role_requests.require_user_session", _fake_session_user),
        patch("web.routes.role_requests.withdraw_role_request", return_value=True) as m,
    ):
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
...
async def test_admin_list_role_requests_defaults_to_pending(app):
    ...
    with (
        ...
        patch("web.routes.admin.list_role_requests", new_callable=AsyncMock, return_value=queue) as m,
    ):
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
...
async def test_admin_list_role_requests_respects_status_filter(app):
    with (
        patch("web.routes.admin._require_admin", _fake_admin),
        patch("web.routes.admin.list_role_requests", new_callable=AsyncMock, return_value=[]) as m,
    ):
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
...
async def test_approve_request_404_when_unknown(app):
    with (
        patch("web.routes.admin._require_admin", _fake_admin),
        patch("web.routes.admin.get_role_request", return_value=None),
    ):
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
...
async def test_approve_request_409_when_already_resolved(app):
    ...
    with (
        patch("web.routes.admin._require_admin", _fake_admin),
        patch("web.routes.admin.get_role_request", return_value=resolved),
    ):
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
...
async def test_reject_request_writes_admin_note(app):
    ...
    with (
        patch("web.routes.admin._require_admin", _fake_admin),
        patch("web.routes.admin.get_role_request", return_value=pending),
        patch("web.routes.admin.review_role_request", return_value=rejected) as m,
    ):
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            r = await client.post(
