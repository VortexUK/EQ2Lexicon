...
from web.auth_deps import require_admin
...
async def test_list_expansions_is_public(app):
    ...
    with patch(
        "web.routes.zones_admin.zones_db.list_featured_raid_expansions",
...
async def test_list_zones_is_public(app):
    with patch(
        "web.routes.zones_admin.zones_db.list_featured_raid_zones",
        return_value=[],
    ):
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
...
async def test_add_expansion_happy_path(app, admin_override):
    with (
        patch(
            "web.routes.zones_admin.zones_db.add_featured_raid_expansion",
            return_value=True,
        ) as add_mock,
        patch("web.routes.zones_admin.invalidate_zones_cache") as invalidate_mock,
    ):
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
...
async def test_delete_expansion_returns_removed_flag(app, admin_override):
    with (
        patch(
            "web.routes.zones_admin.zones_db.remove_featured_raid_expansion",
            return_value=True,
        ) as rm_mock,
        patch("web.routes.zones_admin.invalidate_zones_cache") as invalidate_mock,
    ):
...
async def test_delete_nonexistent_expansion_returns_removed_false(app, admin_override):
    ...
    with (
        ...
        patch("web.routes.zones_admin.invalidate_zones_cache"),
    ):
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
...
async def test_add_zone_happy_path(app, admin_override):
    with (
        patch(
            "web.routes.zones_admin.zones_db.add_featured_raid_zone",
            return_value=_ZONE,
        ) as add_mock,
        patch("web.routes.zones_admin.invalidate_zones_cache") as invalidate_mock,
    ):
...
async def test_delete_zone_happy_path(app, admin_override):
    with (
        patch(
            "web.routes.zones_admin.zones_db.remove_featured_raid_zone",
            return_value=True,
        ) as rm_mock,
        patch("web.routes.zones_admin.invalidate_zones_cache") as invalidate_mock,
    ):
...
def _seed_zones(db_path):
    ...
    from census import zones_db as zdb
...
async def test_get_categories_is_public(app):
    ...
    with patch(
        "web.routes.zones_admin.zones_db.list_featured_raid_categories",
...
async def test_reorder_zones_happy_path(app, admin_override):
    with (
        patch(
            "web.routes.zones_admin.zones_db.reorder_featured_raid_zones",
            return_value=True,
        ) as ro_mock,
        patch("web.routes.zones_admin.invalidate_zones_cache") as invalidate_mock,
    ):
...
async def test_reorder_categories_happy_path(app, admin_override):
    with (
        patch(
            "web.routes.zones_admin.zones_db.reorder_featured_raid_categories",
            return_value=True,
        ) as ro_mock,
        patch("web.routes.zones_admin.invalidate_zones_cache") as invalidate_mock,
    ):
...
async def test_create_category_happy_path(app, admin_override):
    with (
        patch(
            "web.routes.zones_admin.zones_db.create_featured_raid_category",
            return_value=True,
        ) as create_mock,
        patch("web.routes.zones_admin.invalidate_zones_cache") as invalidate_mock,
    ):
...
async def test_create_category_rejects_empty_name(app, admin_override):
    with patch("web.routes.zones_admin.invalidate_zones_cache") as invalidate_mock:
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
...
async def test_delete_category_happy_path(app, admin_override):
    with (
        patch(
            "web.routes.zones_admin.zones_db.delete_featured_raid_category",
            return_value=True,
        ) as delete_mock,
        patch("web.routes.zones_admin.invalidate_zones_cache") as invalidate_mock,
    ):
