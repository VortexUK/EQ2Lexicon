...
async def test_notifications_admin_only_reports_pending_users(app):
    ...
    with (
        ...
        patch("web.routes.notifications.list_pending_users", new=AsyncMock(return_value=fake_pending)),
        patch("web.routes.notifications.get_active_claims", new=AsyncMock(return_value={"approved": []})),
    ):
...
async def test_notifications_officer_reports_guild_claims(app):
    ...
    with (
        ...
        patch("web.routes.notifications.list_pending_users", new=AsyncMock(return_value=[])),
        patch("web.routes.notifications.get_active_claims", new=AsyncMock(return_value={"approved": approved})),
        patch("web.routes.notifications.character_cache") as mock_cache,
        patch("web.routes.notifications._roster_rank_map", new=AsyncMock(return_value=rank_map)),
        patch("web.routes.notifications.list_claims", new=AsyncMock(return_value=pending_claims)),
    ):
...
async def test_notifications_dedupes_claims_across_multi_guild_officer(app):
    ...
    with (
        ...
        patch("web.routes.notifications.get_active_claims", new=AsyncMock(return_value={"approved": approved})),
        patch("web.routes.notifications.character_cache") as mock_cache,
        patch("web.routes.notifications._roster_rank_map", new=_fake_rank_map),
        patch("web.routes.notifications.list_claims", new=AsyncMock(return_value=pending_claims)),
    ):
...
async def test_notifications_admin_who_is_also_officer_combines(app):
    ...
    with (
        ...
        patch("web.routes.notifications.list_pending_users", new=AsyncMock(return_value=fake_pending_users)),
        patch("web.routes.notifications.get_active_claims", new=AsyncMock(return_value={"approved": approved})),
        patch("web.routes.notifications.character_cache") as mock_cache,
        patch("web.routes.notifications._roster_rank_map", new=AsyncMock(return_value=rank_map)),
        patch("web.routes.notifications.list_claims", new=AsyncMock(return_value=pending_claims)),
