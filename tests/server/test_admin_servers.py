...
async def test_put_servers_unknown_world_is_404(app):
    with (
        patch("backend.server.api.admin._require_admin", _fake_admin),
        patch("backend.server.api.admin.list_servers_sync", return_value=[]),
    ):
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
...
async def test_put_servers_updates_settings_and_refreshes_registry(app):
    ...
    with (
        ...
        patch("backend.server.api.admin.server_context.load_registry", mock_reload),
...
async def test_admin_claims_scoped_to_current_world(app):
    ...
    with (
        patch("backend.server.api.admin._require_admin", _fake_admin),
        patch("backend.server.api.admin.list_claims", _mock_list_claims),
...
async def test_put_servers_with_is_default_true_sets_default(app):
    ...
    with (
        ...
        patch("backend.server.api.admin.list_servers_sync", return_value=servers),
        patch("backend.server.api.admin.upsert_server_settings_sync", mock_upsert),
        patch("backend.server.api.admin.set_default_server_sync", mock_set_default),
        patch("backend.server.api.admin.server_context.load_registry", mock_reload),
        patch("backend.server.api.admin.get_server_by_world_sync", return_value=updated_wuoshi),
    ):
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            r = await client.put(
                "/api/admin/servers/Wuoshi",
...
async def test_put_servers_is_default_false_does_not_call_set_default(app):
    ...
    with (
        patch("backend.server.api.admin._require_admin", _fake_admin),
        patch("backend.server.api.admin.list_servers_sync", return_value=servers),
        patch("backend.server.api.admin.upsert_server_settings_sync", MagicMock()),
        patch("backend.server.api.admin.set_default_server_sync", mock_set_default),
        patch("backend.server.api.admin.server_context.load_registry", MagicMock()),
        patch("backend.server.api.admin.get_server_by_world_sync", return_value=updated),
    ):
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
