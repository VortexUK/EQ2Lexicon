...
async def test_list_parses_resolves_uploader_discord_identity(app):
    ...
    with (
        ...
        patch(
            "backend.server.api.parses.list.users_db.get_display_names_for_discord_ids",
            fake_resolve,
        ),
    ):
...
async def test_list_parses_non_plugin_upload_has_null_uploader_identity(app):
    ...
    with (
        patch("backend.server.api.parses.list._require_user", _fake_user),
        patch("backend.server.api.parses.list._list_encounters_sync", fake_list_sync),
        patch("backend.server.api.parses.list.users_db.get_display_names_for_discord_ids", fake_resolve),
    ):
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            r = await client.get("/api/parses")
