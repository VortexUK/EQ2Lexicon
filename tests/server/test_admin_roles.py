...
async def test_list_users_includes_roles(app):
    ...
    with (
        ...
        patch("backend.server.api.admin.list_all_users", return_value=users),
        patch("backend.server.api.admin.list_role_assignments", return_value=role_map),
    ):
...
async def test_grant_role_writes_with_admin_as_grantor(app):
    ...
    with (
        patch("backend.server.api.admin._require_admin", _fake_admin),
        patch("backend.server.api.admin.grant_role", return_value=True) as m,
    ):
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
...
async def test_grant_role_idempotent(app):
    ...
    with (
        patch("backend.server.api.admin._require_admin", _fake_admin),
        patch("backend.server.api.admin.grant_role", return_value=False),
    ):
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            r = await client.post("/api/admin/users/u1/roles/contributor")
...
async def test_revoke_role_user_didnt_have_it_is_404(app):
    ...
    with (
        patch("backend.server.api.admin._require_admin", _fake_admin),
        patch("backend.server.api.admin.revoke_role", return_value=False),
    ):
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
...
async def test_revoke_role_happy_path(app):
    with (
        patch("backend.server.api.admin._require_admin", _fake_admin),
        patch("backend.server.api.admin.revoke_role", return_value=True),
    ):
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            r = await client.delete("/api/admin/users/u1/roles/contributor")
    assert r.status_code == 200
