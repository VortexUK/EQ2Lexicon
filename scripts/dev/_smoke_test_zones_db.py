...
# ── zone_bosses ───────────────────────────────────────────────────────
print("\n--- zone_bosses (when scrape loaded) ---")
with sqlite3.connect(DB) as conn:
    n_bosses = conn.execute("SELECT COUNT(*) FROM zone_bosses").fetchone()[0]
print(f"  zone_bosses rows: {n_bosses}")
if n_bosses > 0:
    # FK integrity check
    with sqlite3.connect(DB) as conn:
        orphan = conn.execute(
            "SELECT COUNT(*) FROM zone_bosses b WHERE NOT EXISTS (SELECT 1 FROM zones z WHERE z.id = b.zone_id)"
        ).fetchone()[0]
    check("zone_bosses has no orphan zone_ids", orphan == 0, f"{orphan} orphans")

    # Spot-check Veeshan's Peak — should have the canonical roster
    vp = zones_db.list_bosses_for_zone("Veeshan's Peak")
    check("Veeshan's Peak has bosses", len(vp) > 5, f"got {len(vp)} bosses: {[b['mob_name'] for b in vp[:3]]}...")

    # find_by_name should now hydrate bosses array
    vp_zone = zones_db.find_by_name("Veeshan's Peak")
    check(
        "find_by_name('Veeshan's Peak') includes bosses array",
        vp_zone is not None and "bosses" in vp_zone and len(vp_zone["bosses"]) > 0,
        f"bosses count: {len(vp_zone['bosses']) if vp_zone else 'n/a'}",
    )

    # Reverse lookup
    mayong_zones = zones_db.find_zones_by_boss("Mayong Mistmoore (Instanced)")
    check(
        "find_zones_by_boss resolves to at least one zone",
        len(mayong_zones) >= 1,
        f"got {[z['name'] for z in mayong_zones]}",
    )

    # Alias-resolved lookup should still find bosses (Veeshan's Peak's
    # bosses should also be reachable through any alias if one exists)
    # Mostly a sanity check that the helper handles alias resolution.
    ...
    print("  (skipping boss-specific checks — scrape data not loaded yet)")
